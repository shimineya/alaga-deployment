...
export const NotificationPanel: React.FC<NotificationPanelProps> = ({
  ...
  // This line prevents the "Cannot read properties of undefined (reading 'filter')" crash
  ...
  const getMaxHeight = () => {
    ...
    if (notificationCount <= 3) return 'auto';
    if (notificationCount <= 6) return 'auto';
    return `${Math.min(500, maxViewportHeight)}px`;
  };
  ...
  const getAlertIcon = (type: Alert['type']) => {
    switch (type) {
      case 'bedwetting': return <Droplets className="w-4 h-4" />;
      case 'vital_signs': return <Activity className="w-4 h-4" />;
      case 'device': return <Wifi className="w-4 h-4" />;
  ...
  const getSeverityColor = (severity: Alert['severity']) => {
    switch (severity) {
      case 'critical': return { bg: 'rgba(217, 83, 79, 0.1)', border: 'var(--status-critical)', text: 'var(--status-critical)' };
      case 'warning': return { bg: 'rgba(240, 173, 78, 0.1)', border: 'var(--status-warning)', text: 'var(--status-warning)' };
      default: return { bg: 'rgba(77, 184, 179, 0.1)', border: 'var(--status-normal)', text: 'var(--status-normal)' };
    }
  };
  ...
  const getSeverityBadge = (severity: Alert['severity']) => {
    ...
    return <Badge className={`${colors[severity]} text-xs`}>{severity.toUpperCase()}</Badge>;
  };
    ...
    <div className="relative">
      ...
      >
        ...
        {unacknowledgedAlerts.length > 0 && (
          <div className="absolute -top-1 -right-1 w-5 h-5 flex items-center justify-center rounded-full text-white text-xs" style={{ backgroundColor: 'var(--status-critical)' }}>
      ...
      {isOpen && (
        <div
          ...
          style={{ maxHeight: getMaxHeight(), boxShadow: '0 10px 40px rgba(77, 184, 179, 0.15)', animation: 'slideDown 200ms ease-out' }}
        >
          <div className="px-4 py-3 border-b border-border flex items-center justify-between sticky top-0 z-10 bg-card" style={{ backgroundColor: 'var(--teal-50)' }}>
            <div className="flex items-center gap-2">
              ...
              {notificationCount > 0 && <Badge className="bg-[var(--teal-600)] text-white text-xs">{notificationCount}</Badge>}
            </div>
            <button onClick={() => setIsOpen(false)} className="p-1 rounded hover:bg-muted transition-colors"><X className="w-4 h-4" /></button>
          </div>
          ...
          {unacknowledgedAlerts.length === 0 ? (
            ...
            <>
              <div className="relative">
                ...
                {showMoreIndicator && <div className="absolute bottom-0 left-0 right-0 h-12 pointer-events-none" style={{ background: 'linear-gradient(to bottom, transparent, var(--card))' }} />}
              </div>
              {showMoreIndicator && hiddenCount > 0 && (
                <div className="px-4 py-2 text-center border-t border-border bg-muted/50">
                  <div className="flex items-center justify-center gap-1 text-xs text-muted-foreground"><ChevronDown className="w-3 h-3 animate-bounce" /><span>{hiddenCount} more notification{hiddenCount !== 1 ? 's' : ''} below</span></div>
                </div>
              )}
              <div className="px-4 py-3 border-t bg-card sticky bottom-0 z-10" style={{ boxShadow: '0 -4px 6px -1px rgba(0, 0, 0, 0.05)' }}>
                <Button variant="outline" size="sm" className="w-full" onClick={handleMarkAllRead} style={{ borderColor: 'var(--teal-300)', color: 'var(--teal-700)' }}>
      ...
      <style>{`
        ...
        @keyframes slideInNotification { from { opacity: 0; transform: translateX(10px); } to { opacity: 1; transform: translateX(0); } }
        .custom-scrollbar::-webkit-scrollbar { width: 6px; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background-color: rgba(77, 184, 179, 0.3); border-radius: 3px; }
      `}</style>
    </div>
  );
};