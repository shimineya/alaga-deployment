...
export const LoginEmailVerification: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loginData = sessionStorage.getItem('loginPendingVerification');
    if (!loginData) {
      navigate('/login');
      return;
    }
  ...
  const verifyCode = async (enteredCode: string) => {
    ...
    setTimeout(async () => {
      if (enteredCode === correctCode) {
        ...
        // Get login credentials and complete login
        const loginData = sessionStorage.getItem('loginPendingVerification');
        if (loginData) {
          ...
          sessionStorage.removeItem('loginPendingVerification');
        ...
        if (newAttempts >= 5) {
          setIsLocked(true);
          setLockTimeLeft(15 * 60); // 15 minutes for login attempts
          toast.error('Too many failed attempts. Account locked for 15 minutes for your security.');
        } else {
  ...
  const handleResendCode = () => {
    if (!canResend && !codeExpired) return;
  ...
  const loginData = sessionStorage.getItem('loginPendingVerification');
  const userEmail = loginData ? JSON.parse(loginData).email : '';
    ...
    <div className="min-h-screen flex items-center justify-center p-6" style={{ backgroundColor: '#F0FAF9' }}>
      <Card className="w-full max-w-md border-0" style={{ boxShadow: '0 4px 20px rgba(0, 0, 0, 0.1)' }}>
        <CardHeader className="text-center pb-6">
          ...
          <p className="text-sm mt-1" style={{ color: '#7DD3C0' }}>
            {userEmail}
          </p>
        </CardHeader>
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <ul className="text-xs space-y-1" style={{ color: '#7F8C8D' }}>
                  ...
                  <li>• Account locks for 15 minutes after 5 failed attempts</li>
