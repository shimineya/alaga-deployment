...
UserSession account(int id, String role, {String token = 'token'}) => UserSession(
  id: id, username: role, email: 'shared@example.test', role: role,
  name: role, token: token,
);
...
void main() {
  ...
  test('each account opts in separately and logout preserves both enrollments', () async {
    ...
    expect((await SessionManager.loadBiometricSessions()).map((s) => s.id), [1, 2]);
  });

  test('refreshing or disabling one account leaves the other account intact', () async {
  ...
  test('legacy enrollment migrates without enrolling the newly signed-in account', () async {
    FlutterSecureStorage.setMockInitialValues({
