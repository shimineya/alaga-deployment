                  ...
                  Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Text(
                            UserSession.current?.name?.isNotEmpty == true ? UserSession.current!.name : (UserSession.current?.username ?? "User"),
                          ...
                          Container(
                            ...
                            child: Text(UserSession.current?.role?.toUpperCase() ?? "CAREGIVER", style: const TextStyle(color: Colors.blue, fontSize: 10, fontWeight: FontWeight.bold)),
                          ),
                        ],
                      ),
                      const SizedBox(height: 4),
                      _iconLabel(Icons.email_outlined, UserSession.current?.email ?? "Not configured"),
                      _iconLabel(Icons.calendar_today_outlined, "Joined Today"),
                    ],
                  ),
                ],
              ),
            ...
            _buildSectionCard(
              child: Column(
                children: [
                  Row(
                    children: [
                      Expanded(child: _buildDataField("Display Name", UserSession.current?.name ?? "N/A")),
                      const SizedBox(width: 15),
                      Expanded(child: _buildDataField("Username", UserSession.current?.username ?? "N/A")),
                    ],
                  ),
                  const SizedBox(height: 15),
                  Row(
                    children: [
                      Expanded(child: _buildDataField("Email Address", UserSession.current?.email ?? "N/A")),
                      const SizedBox(width: 15),
                      Expanded(child: _buildDataField("Role", UserSession.current?.role.toUpperCase() ?? "CAREGIVER")),
                    ],
                  ),
                ],
              ),
