...
export default function SecurityControls() {
  ...
  const fetchData = async () => {
    try {
        ...
        const res = await fetch('http://localhost:3000/api/admin/security', {
  ...
  const handleBanIp = async () => {
    ...
    try {
        const token = localStorage.getItem('token');
        const res = await fetch('http://localhost:3000/api/admin/security/ip-ban', {
            method: 'POST',
  ...
  const saveRateLimit = async () => {
    try {
        const token = localStorage.getItem('token');
        await fetch('http://localhost:3000/api/admin/security/rate-limit', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
